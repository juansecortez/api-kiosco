-- Procedimiento Almacenado para Detalles de Acceso a Sistemas de Información
CREATE PROCEDURE sp_DetallesAccesoSistemas
	@tipo_id INT,
	@sol_usuarioidSolicitante VARCHAR(15),
	@sol_fechaSolicitud DATE,
	@sis_direccion varchar(50),
	@sis_gerencia varchar(50),
	@sis_jefatura varchar(50),
	@sis_nombreSistema varchar(50),
	@sis_funcionDesarrollar varchar(250)
	
AS
BEGIN

	DECLARE @sol_id INT

	-- Ejecutar el procedimiento almacenado de encabezado_solicitudes para insertar el encabezado
	EXEC sp_InsertarSolicitud @tipo_id, @sol_usuarioidSolicitante, @sol_fechaSolicitud							

	-- Obtener el sol_id generado para la solicitud recién insertada
	SET @sol_id = @@IDENTITY

	-- Insertar datos en detalles_altasUsers
	INSERT INTO detalles_accesoSistemas (sis_idSolicitud, sis_direccion, sis_gerencia, 
									 sis_jefatura, sis_nombreSistema, sis_funcionDesarrollar)
    VALUES (@sol_id, @sis_direccion, @sis_gerencia,@sis_jefatura, @sis_nombreSistema, 
			@sis_funcionDesarrollar)

END
